...
// 1. IMPORTAR SERVICIOS DE FIREBASE
...
function LoginForm() {
  ...
  // Función auxiliar para solicitar y guardar el token FCM en Supabase
  ...
  try {
    if (typeof window !== 'undefined' && 'Notification' in window) {
      ...
      if (permission === 'granted') {
        console.log('Permiso concedido. Solicitando token a FCM...');
        ...
        if (currentToken) {
          // Imprimimos el token en la consola del navegador
          console.log('✅ FCM Token recibido con éxito:', currentToken);
          ...
          if (error) {
            console.error('❌ Error al guardar el token en Supabase:', error.message);
          } else {
            console.log('✅ Token FCM guardado correctamente en Supabase.');
          }
        } else {
          console.warn('⚠️ No se obtuvo ningún token FCM. Revisa las llaves VAPID o el Service Worker.');
        }
      } else {
    ...
    console.error('❌ Error durante la generación/obtención del token FCM:', err);
  }
};
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // 4. REGISTRAR TOKEN PUSH FCM (Especialmente relevante si el rol es limpieza)
    if (profile.role === 'limpieza') {
      await saveFcmToken(authData.user.id);
    }
