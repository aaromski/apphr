...
import { 
  ...
  TriangleAlert // <-- Icono de alerta SLA
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // Configuración SLA por tipo de habitación + reloj base para los temporizadores
...
// 1. Obtener sesión de usuario actual y sus datos de la tabla profiles
      ...
      if (session?.user) {
        ...
        if (profile) {
          setUserData(profile);
          // Cargar la configuración SLA por tipo de habitación del hotel
          if (profile.hotel_id) {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'UPDATE') {
            ...
            setNotification({
              message: `Habitación ${updatedRoom.room_number} cambió a: ${updatedRoom.status}`,
              visible: true,
            });
  ...
  const handleLogout = async () => {
    ...
    router.push('/'); // Cambia a '/login' si tu ruta de login es esa específicamente
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 bg-white dark:bg-[#111625] p-1 rounded-xl border border-slate-200 dark:border-slate-800 text-xs shadow-sm">
              {['Todos', 'Disponible', 'Ocupada', 'Sucia', 'En Limpieza', 'Limpia/Lista', 'Mantenimiento'].map((status) => (
                <button
                  key={status}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredRooms.map((room) => {
              ...
              // Lógica SLA para limpiezas en curso (se evalúa en cada tick del reloj base)
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-[#111625] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-all hover:shadow-xl relative group shadow-sm ${
                    isCritical ? 'ring-2 ring-rose-500/70 border-rose-500 animate-pulse' : ''
                  }`}
                >
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {/* Menú desplegable con todas las opciones operativas */}
                      <select
                        value={room.status}
                        onChange={(e) => updateRoomStatus(room.id, e.target.value)}
                        className="text-[10px] bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-1 font-semibold text-slate-700 dark:text-slate-300 outline-none"
                      >
                        <option value="Disponible">Disponible</option>
                        <option value="Ocupada">Ocupada</option>
                        <option value="Sucia">Sucia</option>
                        <option value="En Limpieza">En Limpieza</option>
                        <option value="Limpia/Lista">Limpia/Lista</option>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/50">
                    ...
                    {room.status === 'En Limpieza' && startedAt ? (
                      <span
                        ...
                        title={`Limpieza en curso • Límite SLA: ${slaMin} min`}
                      >
                        {isCritical ? <TriangleAlert className="w-3.5 h-3.5" /> : <Clock className="w-3 h-3" />}
                        {formatTimer(elapsed)}
                        {isCritical && (
                          <span className="uppercase text-[9px] tracking-wide">SLA excedido</span>
                        )}
                      </span>
                    ) : room.cleaning_timer ? (
