...
export default function DashboardPage() {
        ...
        async (payload) => {
          if (payload.eventType === 'UPDATE') {
            ...
            if (previousRoom && previousRoom.status === 'Ocupada' && updatedRoom.status === 'Sucia') {
              ...
              try {
                await fetch('https://ntfy.sh/apphr-hotel-152', {
                  method: 'POST',
                  headers: {
                    Title: 'Habitacion sucia',
                    Priority: 'urgent',
                    Tags: 'warning',
                  },
  ...
  }, [publishNotification, userData?.hotel_id]);
  ...
  const filteredRooms = rooms.filter(room => {
    ...
    const matchesZone = !selectedZone || room.zonas?.nombre === selectedZone;
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-[#0b0f19] text-slate-900 dark:text-slate-200 font-sans flex flex-col transition-colors duration-300">
      ...
      {mobileMenuOpen && (
        <div 
          className={c('fixed inset-0 bg-black/50 z-30 lg:hidden')}
          onClick={() => setMobileMenuOpen(false)}
          aria-hidden="true"
        />
      )}

      <header className={c('h-16 border-b border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-[#111625]/80 backdrop-blur-md px-4 lg:px-6 flex items-center justify-between sticky top-0 z-30 transition-colors duration-300')}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className={c('w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center shadow-md shadow-indigo-500/20')}>
        ...
        <div className="flex items-center gap-3">
          <div className={c('hidden md:flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-full text-[11px] font-medium text-emerald-600 dark:text-emerald-400')}>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
            Sincronización en Realtime Activa
          </div>

          <div className="relative hidden sm:block">
            <Search className={c('w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2')} />
            <input
              type="text"
              placeholder="Buscar habitación..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className={c('bg-slate-50 dark:bg-[#0b0f19] border border-slate-200 dark:border-slate-700/80 rounded-xl py-1.5 pl-9 pr-4 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 w-48 focus:w-64 focus:outline-none focus:border-indigo-500 transition-all')}
            />
          </div>
          ...
          <div className="relative" ref={bellMenuRef}>
            ...
            {showNotificationsMenu && (
              <div className={c('absolute right-0 top-full mt-2 w-80 sm:w-[22rem] bg-white/95 dark:bg-[#111625]/95 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl shadow-slate-900/10 dark:shadow-black/40 z-50 overflow-hidden')}>
                <div className={c('flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800/80')}>
                  <div className="flex items-center gap-2">
                    ...
                    {unreadCount > 0 && (
                      <span className={c('text-[10px] font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded-full')}>
                        {unreadCount}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-0.5">
                    <button
                      ...
                      className={c('p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-indigo-500/10 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed')}
                    >
                    ...
                    <button
                      ...
                      className={c('p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed')}
                    >
                ...
                <div className="max-h-96 overflow-y-auto overscroll-contain">
                  {notificationsList.length === 0 ? (
                    ...
                    <ul className={c('divide-y divide-slate-100 dark:divide-slate-800/60')}>
                      {notificationsList.map((item) => (
                        <li
                          key={item.id}
                          className={cn(
                            'px-4 py-3 flex items-start gap-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/40',
                            item.unread ? 'bg-indigo-500/[0.04] dark:bg-indigo-500/[0.06]' : ''
                          )}
                        >
                          <div
                            className={cn(
                              'w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5',
                              item.kind === 'priority' ? 'bg-amber-500/10 text-amber-500' : 'bg-indigo-500/10 text-indigo-500'
                            )}
                          >
                            {item.kind === 'priority' ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <p
                              className={cn(
                                ...
                                item.unread ? 'font-semibold text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-300'
                              )}
                            >
                              {item.message}
                            </p>
          ...
          <div className="flex items-center gap-3 pl-2 border-l border-slate-200 dark:border-slate-800">
            <div className={c('w-8 h-8 rounded-full bg-indigo-600/20 dark:bg-indigo-600/30 text-indigo-600 dark:text-indigo-400 overflow-hidden flex items-center justify-center border border-indigo-500/30 text-xs font-bold')}>
        ...
        <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 ${mobileFiltersOpen ? 'flex' : 'hidden'} lg:flex`}>
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5 bg-white dark:bg-[#111625] p-1 rounded-xl border border-slate-200 dark:border-slate-800 text-xs shadow-sm overflow-x-auto pb-1 no-scrollbar">
              <button
                onClick={() => { setSelectedZone(null); setActiveFilter('Todos'); setMobileFiltersOpen(false); }}
                className={`px-3 py-1 rounded-lg font-medium transition-all whitespace-nowrap ${
                  selectedZone === null && activeFilter === 'Todos'
              ...
              {zonas.map((zona) => (
                <button
                  key={zona}
                  onClick={() => { setSelectedZone(zona); setActiveFilter('Todos'); setMobileFiltersOpen(false); }}
                  className={`px-3 py-1 rounded-lg font-medium transition-all whitespace-nowrap ${
                    selectedZone === zona
                      ? 'bg-indigo-600 text-white shadow-sm'
                      : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
            ...
            <div className={c('flex items-center gap-2 text-xs font-medium bg-white dark:bg-[#111625] border border-slate-200 dark:border-slate-800/80 p-1.5 rounded-xl shadow-sm flex-wrap')}>
              <span className={c('bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-lg')}>
                <strong className="font-bold">{counts.disponibles}</strong> Disponibles
              </span>
              <span className={c('bg-rose-500/10 text-rose-600 dark:text-rose-400 px-2.5 py-1 rounded-lg')}>
              ...
              <span className={c('bg-amber-500/10 text-amber-600 dark:text-amber-400 px-2.5 py-1 rounded-lg')}>
                <strong className="font-bold">{counts.limpieza}</strong> En Limpieza
              </span>
              <span className={c('bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-2.5 py-1 rounded-lg')}>
                <strong className="font-bold">{counts.limpiaLista}</strong> Limpia/Lista
              </span>
              <span className={c('bg-purple-500/10 text-purple-600 dark:text-purple-400 px-2.5 py-1 rounded-lg')}>
            ...
            {filteredRooms.map((room) => {
              ...
              return (
                <div
                  key={room.id}
                  className={cn(
                    'bg-white dark:bg-[#111625] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-all hover:shadow-xl relative group shadow-sm',
                    isCritical
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      <span className={c('text-[10px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700/50')}>
                        {room.room_type_config?.room_type || 'Estándar'}
                      </span>
                      {room.is_priority && (
                        <span className={c('text-[10px] font-bold bg-amber-500/15 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-md border border-amber-500/30 flex items-center gap-1')}>
                  ...
                  <div className={c('flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/50')}>
                    ...
                    {room.status === 'En Limpieza' && startedMs ? (
                      ...
                      >
                        {isCritical ? <TriangleAlert className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                        {formatTimer(elapsed)}
                        {isCritical && (
